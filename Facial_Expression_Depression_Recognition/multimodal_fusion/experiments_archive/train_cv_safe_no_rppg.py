"""
train_cv_safe_no_rppg.py

Same leak-free cross-validation + ensembling as train_cv_safe.py, but
WITHOUT the rPPG modality (visual + clip + smile only). Used to fairly
compare against the WITH-rPPG cross-validation result (ensemble MAE
7.73), using the same trustworthy method both times.

Train+Dev pool (197 people) used for the 5 folds. Testing split (100
people) is never touched until the final, one-time evaluation.

Run:
    python train_cv_safe_no_rppg.py
"""
...
def main():
    ...
    for fold_index, (train_idx, val_idx) in enumerate(kfold.split(indices, groups=group_ids)):
        ...
        fold_path = config.MOE_FUSION_BRANCH_DIR / f"cv_safe_norppg_fold{fold_index + 1}.pt"
    ...
    with_rppg = {"MAE": 7.7283, "RMSE": 10.0032, "PCC": 0.5145, "CCC": 0.4488}

    print("\n================ FINAL, HONEST COMPARISON (both cross-validated + ensembled) ================")
    print(f"{'Metric':<8}{'WITH rPPG (ensemble)':<24}{'WITHOUT rPPG (ensemble)':<26}")
    for key in ["MAE", "RMSE", "PCC", "CCC"]:
        print(f"{key:<8}{with_rppg[key]:<24.4f}{ensemble_metrics[key]:<26.4f}")
