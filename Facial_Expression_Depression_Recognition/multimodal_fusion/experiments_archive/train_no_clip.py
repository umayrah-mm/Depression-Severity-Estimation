...
Ablation: trains and evaluates the MoE fusion model WITHOUT the CLIP
modality (visual + rppg + smile/audio only), using the SAME official
Training/Development/Testing split as evaluate.py, so the result is
directly comparable to your current MAE 8.15 (4-modality) result.
...
# ---- The only real change: drop "clip" from the modality list ----
NO_CLIP_MODALITIES = ["visual", "rppg", "smile"]
models.MODALITY_ORDER = NO_CLIP_MODALITIES
...
def run_one_epoch(model, loader, loss_fn, optimizer, device, train=True):
    ...
    with context:
        for batch in loader:
            inputs = {name: batch[name].to(device) for name in NO_CLIP_MODALITIES}
...
def main():
    ...
    print(f"Modalities used: {NO_CLIP_MODALITIES}  (CLIP removed)")
    ...
    print(f"Trainable parameters: {n_params:,} ({n_params/1e6:.3f} M)  "
          f"(expect fewer than your 4-modality model, since CLIP's projector/expert are gone)")
    ...
    print(f"{'Metric':<8}{'Original (4 modalities, w/ CLIP)':<34}{'No CLIP (3 modalities)':<25}")
    for key in ["MAE", "RMSE", "PCC", "CCC"]:
        print(f"{key:<8}{original[key]:<34.4f}{new_metrics[key]:<25.4f}")

    output_path = config.MOE_FUSION_BRANCH_DIR / "best_model_no_clip.pt"
