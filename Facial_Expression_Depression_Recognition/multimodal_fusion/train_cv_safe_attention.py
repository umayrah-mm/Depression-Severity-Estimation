"""
train_cv_safe_attention.py

Leak-free cross-validation + ensembling for the ATTENTION fusion model
(models_attention.py), using the exact same safe methodology as
train_cv_safe.py: 5 folds trained on Training+Development people only
(197 total), Testing split (100 people) never touched until the final,
one-time evaluation at the end.

Extra safety for this model specifically (since attention has more
...
def main():
    ...
    for fold_index, (train_idx, val_idx) in enumerate(kfold.split(indices, groups=group_ids)):
        ...
        # Report the train/val gap for THIS fold - our overfitting check
        ...
        fold_path = config.MOE_FUSION_BRANCH_DIR / f"cv_safe_attention_final_fold{fold_index + 1}.pt"
    ...
    moe_ensemble = {"MAE": 7.7283, "RMSE": 10.0032, "PCC": 0.5145, "CCC": 0.4488}
